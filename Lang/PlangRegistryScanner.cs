...
namespace Polaris.Lang
{
    ...
    /// 发现＋调用交给 <see cref="ContentRegistrarScanner"/>；扫描同时是 key 冲突判定现场：每个注册类调用前
    /// 先点名所属程序集（<see cref="PlangConflictGuard.CurrentSource"/>），全部结束后由
    /// <see cref="PlangConflictGuard.Seal"/> 汇总处置。
    /// </summary>
    internal static class PlangRegistryScanner
    {
        static bool scanned;

        /// <summary>由 <see cref="PolarisLangComponent.Start"/> 调用一次。</summary>
        internal static void ScanAll()
        {
            ...
            int count = ContentRegistrarScanner.ScanAndRun<PlangAutoRegistrationAttribute, IPlangRegistrar>(
                (registrar, type) =>
                {
                ...
                (e, type) => Plugin.Logger.LogError($"[PolarisLang] Failed to auto-register {type.FullName}; skipped: {e}"));
